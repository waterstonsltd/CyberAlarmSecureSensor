...
| Metric | Type | Tags | Description |
|--------|------|------|-------------|
| `upload.build_info` | Gauge | `version` | Always 1. Use the `version` tag in Grafana to display or filter by build version |
| `upload.selector.files_selected` | Counter | — | Files moved from the logs folder into the processing folder |
| `upload.selector.duration` (ms) | Histogram | — | Time taken to complete a file selection pass |
| `upload.grouping.files_processed` | Counter | — | Input files successfully processed during source grouping |
| `upload.grouping.files_failed` | Counter | — | Input files that failed during source grouping |
| `upload.grouping.events_written` | Counter | — | Events written to grouped output files |
| `upload.grouping.output_files_created` | Counter | — | Grouped output files created (one per source/ingestion-method combination per cycle) |
| `upload.grouping.duration` (ms) | Histogram | — | Time taken to complete a source grouping pass |
| `upload.bundling.files_bundled` | Counter | — | Files successfully encrypted, compressed, and bundled for upload |
| `upload.bundling.files_failed` | Counter | — | Files that failed during the bundling step |
| `upload.bundling.duration` (ms) | Histogram | — | Time taken to complete a bundling pass |
