...
Binding to port 514 may require administrative rights or a different port plus a firewall or device-side forward. If the service starts but syslog sources cannot connect, check the Windows Firewall rules and whether another process is already bound to port 514.
