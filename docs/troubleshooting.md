...
Reads local state, health, and log files, runs live TCP connectivity probes, and prints a structured report. Exits immediately without starting the sensor.

**Windows** (run from the install directory, or with the full path):

```powershell
& "C:\Program Files\CyberAlarmSecureSensor\CyberAlarm.SyslogRelay.ConsoleApp.exe" --diagnostics
```

**Docker** (run against the running container):

```bash
docker exec syslog-relay /app/CyberAlarm.SyslogRelay.ConsoleApp --diagnostics
```

If the container is in a restart loop and `docker exec` fails, run diagnostics directly against the data directory without starting the relay:
...
```powershell
& "C:\Program Files\CyberAlarmSecureSensor\CyberAlarm.SyslogRelay.ConsoleApp.exe" --support-bundle
```

**Docker:**

```bash
docker exec syslog-relay /app/CyberAlarm.SyslogRelay.ConsoleApp --support-bundle
# Then copy the file out:
docker cp syslog-relay:/tmp/cyberalarm-support-<timestamp>.zip ./
```

If the container is in a restart loop, use a temporary container instead:
