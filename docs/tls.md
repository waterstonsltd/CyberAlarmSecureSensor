...
**[Let's Encrypt](https://letsencrypt.org/)** (via [Certbot](https://certbot.eff.org/)) is a good free option for the server certificate if the relay host has a public hostname and can complete an ACME challenge. Note that Let's Encrypt issues server certificates only — it cannot issue client certificates for mutual TLS. Once issued, convert the certificate to PKCS#12 with:
