...
### Windows Firewall rule for port 6514

The installer automatically opens ports 514 (TCP and UDP) in Windows Firewall, but it does **not** open port 6514 for TLS — TLS is opt-in and configured after install. You must add the rule manually after enabling TLS:

```powershell
New-NetFirewallRule `
    -DisplayName "CyberAlarm Secure Sensor - Syslog TLS (TCP)" `
    -Direction Inbound `
    -Action Allow `
    -Protocol TCP `
    -LocalPort 6514 `
    -Profile Any `
    -Description "Allow inbound TLS syslog traffic to CyberAlarm Secure Sensor"
```

If you have also disabled the plaintext listeners (`AllowPlaintextListenersWhenTlsEnabled` is `false`, which is the default), you can remove the port 514 rules to prevent unencrypted ingest:

```powershell
Remove-NetFirewallRule -DisplayName "CyberAlarm Secure Sensor - Syslog (TCP)"
Remove-NetFirewallRule -DisplayName "CyberAlarm Secure Sensor - Syslog (UDP)"
```
