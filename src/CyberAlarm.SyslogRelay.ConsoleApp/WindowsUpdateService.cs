...
internal sealed class WindowsUpdateService : BackgroundService
{
    ...
    public WindowsUpdateService(
        ...
        ILogger<WindowsUpdateService> logger)
    {
    ...
    private async Task PerformVelopackCheckAndUpdateAsync(bool allowPreRelease, Version? approvedVersion, Version? installedVersion, CancellationToken cancellationToken)
    {
        ...
        logger.LogInformation("Applying Windows update {Version} and requesting graceful shutdown.", update.TargetFullRelease.Version);

        // restart: false — the OnAfterUpdateFastCallback hook (WindowsHost.Install) starts the service after the update is applied. Letting Velopack also restart would cause a double start.
        updateManager.WaitExitThenApplyUpdates(update.TargetFullRelease, silent: true, restart: false);
        applicationManager.StopApplication($"Windows update to version {update.TargetFullRelease.Version}");
    }
