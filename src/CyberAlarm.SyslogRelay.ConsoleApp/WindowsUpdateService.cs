...
internal sealed class WindowsUpdateService : BackgroundService
{
    ...
    private async Task CheckForUpdatesAsync(CancellationToken cancellationToken)
    {
        try
        {
            var approvedVersion = await GetApprovedVersionAsync(cancellationToken).ConfigureAwait(false);
            ...
            if (installedVersion >= approvedVersion)
            {
                logger.LogDebug(
            ...
            if (targetVersion != approvedVersion)
            {
                logger.LogWarning(
