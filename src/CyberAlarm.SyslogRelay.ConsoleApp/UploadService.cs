    ...
    ILogger<UploadService> logger) : BackgroundService
{
    ...
    private async Task<bool> RunUploadCycleAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var uploadResult = await _pipeline.SecureUploader.UploadFilesAsync(cancellationToken);

            stopwatch.Stop();

            if (uploadResult.HasUploads)
            {
                _logger.LogInformation(
                    "Scheduled upload completed in {Elapsed}: {FilesUploaded} file(s) uploaded, {FilesFailed} failed",
                    stopwatch.Elapsed.ToString(@"hh\:mm\:ss"),
                    uploadResult.FilesUploaded,
                    uploadResult.FilesFailed);
            }
            else
            {
                _logger.LogInformation(
                    "Scheduled upload completed in {Elapsed}: no files to upload (staying on initial interval)",
                    stopwatch.Elapsed.ToString(@"hh\:mm\:ss"));
            }

            return uploadResult.HasUploads;
        }
        catch (Exception ex)
        {
            stopwatch.Stop();
