...
internal static class Program
{
    ...
    private static async Task RunDiagnostics(IServiceScope scope)
    {
        var diagnosticsService = scope.ServiceProvider.GetRequiredService<DiagnosticsService>();
        await diagnosticsService.RunAsync(Console.Out, CancellationToken.None);
        Environment.Exit(0);
    }
