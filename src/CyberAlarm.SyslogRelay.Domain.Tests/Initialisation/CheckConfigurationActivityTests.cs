...
public sealed class CheckConfigurationActivityTests
{
    ...
    [Fact]
    public async Task RunAsync_should_fail_when_api_base_url_is_missing()
    {
        // Arrange
        var options = new RelayOptionsBuilder()
            .WithApiBaseUrl(string.Empty)
            .Build();
        ...
        // Assert
        Assert.True(result.IsFailed);
        Assert.Equal("API base URL is not configured.", result.ErrorMessage);
    }

    [Fact]
    public async Task RunAsync_should_fail_when_file_watcher_is_enabled_without_a_drop_path()
    {
        // Arrange
        var options = new RelayOptionsBuilder()
        ...
        // Assert
        Assert.True(result.IsFailed);
        Assert.Equal("File watcher is enabled but FileWatcherDropPath is not configured.", result.ErrorMessage);
    }
