...
public sealed class FileBundlerTests
{
    ...
    private readonly EventsMetaData _eventsMetaData = new("test-ingestion", "test-source", 1, 0, 0, 0);
    ...
    [Fact]
    public async Task LoadsEventsMetaData()
    {
        var metaDataFilePath = "metadata.json";
        _builder.FileManager
            .GetEventsMetaDataFilePath()
            .Returns(metaDataFilePath);
        var systemUnderTest = _builder
            .WithEventsMetaData([("file1.log", _eventsMetaData)])
            .Build();
        ...
        await _builder.FileManager
            .Received()
            .DeserialiseFromFileAsync<Dictionary<string, EventsMetaData>>(metaDataFilePath, Arg.Any<CancellationToken>());
        _builder.FileManager
            .Received()
    ...
    [Fact]
    public async Task LogsErrorWhenLoadingEventsMetaDataThrows()
    {
        _builder.FileManager
        ...
        Assert.Contains(
            _builder.Logger.ReceivedLogs(),
            log => log.LogLevel == LogLevel.Error &&
                log.Message is not null &&
                log.Message.Contains("Failed to load metadata from '"));
    }

    [Fact]
    public async Task ThrowsWhenEventsMetaDataIsMissingForInputFile()
    {
        var systemUnderTest = _builder
            .WithSourceFiles(["source1/file1.log"])
            .Build();

        var exception = await Assert.ThrowsAsync<InvalidOperationException>(async () => await systemUnderTest.BundleFilesAsync(CancellationToken.None));

        await _builder.FileManager
            .Received()
            .DeserialiseFromFileAsync<Dictionary<string, EventsMetaData>>(Arg.Any<string>(), Arg.Any<CancellationToken>());
        Assert.Equal("Events metadata entry missing for file 'file1.log'.", exception.Message);
    }
    ...
    {
        var systemUnderTest = _builder
            .WithSourceFiles(["source1/file1.log", "source2/file2.log", "source3/file3.log"])
            .WithEventsMetaData([("file1.log", _eventsMetaData), ("file2.log", _eventsMetaData), ("file3.log", _eventsMetaData)])
            .Build();
