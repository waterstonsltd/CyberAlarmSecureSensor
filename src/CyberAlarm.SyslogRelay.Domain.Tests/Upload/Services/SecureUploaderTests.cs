...
public sealed class SecureUploaderTests
{
    ...
    public async Task CreatesSecureFtpClientIfThereAreFilesToUpload(string[] files, int expectedCreationCount)
    {
        ...
        _builder.SecureFtpClientFactory
            ...
            .Create("ftptestserver1.blob.core.windows.net", "ftptestserver1.testUserName", "test private key",
                Arg.Is<IReadOnlyList<string>>(fps => fps.Count == 1 && fps[0] == "test host fingerprint"));
    }
    ...
    public async Task PassesBothFingerprintsToFtpClientWhenSecondaryIsSet()
    {
        ...
        _builder.SecureFtpClientFactory
            ...
            .Create(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(),
                Arg.Is<IReadOnlyList<string>>(fps =>
                    fps.Count == 2 &&
                    fps[0] == "primary fingerprint" &&
                    fps[1] == "secondary fingerprint"));
    }
}
