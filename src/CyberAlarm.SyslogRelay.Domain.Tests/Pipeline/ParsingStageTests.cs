...
public sealed class ParsingStageTests
{
    ...
    [Fact]
    public async Task Should_set_is_ignored_when_log_contains_ignored_substring()
    {
        // Arrange
        var parser = Substitute.For<IParser>();
        var input = new PatternMatchingStageOutput(
            SyslogEvent.FromFile("root", "type=\"siem\" extra=data"),
            new PatternMatchResult("Fortigate", parser, ["type=\"siem\""]));  

        var outputs = new List<ParsingStageOutput>();
        var unitUnderTest = CreateStage(outputs, CreateLogger());
    ...
    [Fact]
    public async Task Should_not_set_is_ignored_when_log_does_not_contain_ignored_substring()
    {
        // Arrange
        var parser = Substitute.For<IParser>();
        parser.Parse(Arg.Any<string>()).Returns(Result.Fail<ParseResult>("fail"));
        var input = new PatternMatchingStageOutput(
            SyslogEvent.FromFile("root", "type=\"traffic\" extra=data"),
            new PatternMatchResult("Fortigate", parser, ["type=\"siem\""]));  
        ...
        parser.Received().Parse("type=\"traffic\" extra=data");
    }
        ...
        TimeProvider? timeProvider = null)
    {
        ...
        return new ParsingStage(
            new PipelineStageServices(applicationManager, healthCheckService, Options.Create(new PipelineOptions()), new PipelineMetrics(new TestMeterFactory())),
            logger,
            timeProvider)
        {
            NextStage = nextStage,
