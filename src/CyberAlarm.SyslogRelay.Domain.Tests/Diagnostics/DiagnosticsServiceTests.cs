...
public sealed class DiagnosticsServiceTests : IDisposable
{
    ...
    public async Task RunAsync_output_contains_header()
    {
        ...
        // Act
        await sut.RunAsync(output, verbose: true, CancellationToken.None);
    ...
    [Fact]
    public async Task RunAsync_focused_always_shows_full_hint_footer()
    {
        // Arrange — any state; the --full hint should always appear in focused mode
        var sut = _builder.Build();
        var output = new StringWriter();

        // Act — focused mode (default, no --full)
        await sut.RunAsync(output, verbose: false, CancellationToken.None);
    ...
    [Fact]
    public async Task RunAsync_focused_shows_issue_when_upload_blocked()
    {
        // Arrange — upload blocked should surface even in focused mode
        _builder.WithState(new RelayStateBuilder().WithIsUploadBlocked(true).Build());
        var sut = _builder.Build();
        var output = new StringWriter();
        ...
        // Assert
        var text = output.ToString();
        Assert.Contains("fatal authentication", text);
        Assert.DoesNotContain("All checks passed", text);
    }
}
