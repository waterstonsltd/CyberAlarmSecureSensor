...
internal sealed class FileBundlerBuilder
{
    ...
    public FileBundler Build() =>
        new(FileManager, EventBundler, PlatformService, StatusService, RsaKeyProvider, Options.Create(_relayOptions), new UploadMetrics(new TestMeterFactory(), Options.Create(_relayOptions)), Logger);
}
