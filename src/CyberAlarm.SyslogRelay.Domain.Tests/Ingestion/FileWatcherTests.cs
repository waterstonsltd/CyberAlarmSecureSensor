...
public sealed class FileWatcherTests
{
    ...
    public async Task StartAsync_should_stop_periodic_operation_when_drop_path_is_not_set()
    {
        ...
        // Assert
        Assert.Equal("File watcher is enabled but no drop path is configured.", exception.Message);
    }

    [Fact]
    public async Task StartAsync_should_throw_when_drop_path_does_not_exist_and_file_watcher_is_enabled()
    {
        // Arrange
        var dropPath = "drop";
        var options = new RelayOptionsBuilder()
        ...
        // Assert
        Assert.Equal("File watcher drop path does not exist.", exception.Message);
    }

    [Fact]
    public async Task StartAsync_should_stop_periodic_operation_when_file_watcher_is_disabled()
    {
        // Arrange
        var options = new RelayOptionsBuilder()
            .WithFileWatcherEnabled(false)
            .Build();
        ...
        Assert.Contains((LogLevel.Information, "File watcher is disabled."), _builder.Logger.ReceivedLogs());
    }
    ...
    public async Task StartAsync_should_start_periodic_operation()
    {
        // Arrange
        var unitUnderTest = _builder
            .WithOptions(new RelayOptionsBuilder().WithFileWatcherEnabled(true).Build())
            .Build();
