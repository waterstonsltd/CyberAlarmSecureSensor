...
public sealed class UdpListenerTests
{
    ...
    public async Task StartAsync_should_unregister_health_check_when_tls_is_enabled_and_plaintext_override_is_not_set()
    {
        ...
        healthCheckService.GetHealthToken(nameof(UdpListener)).Returns(healthToken);
        var logger = Substitute.For<ILogger<UdpListener>>();
        ...
        var unitUnderTest = new UdpListener(applicationManager, healthCheckService, Options.Create(options), logger);
