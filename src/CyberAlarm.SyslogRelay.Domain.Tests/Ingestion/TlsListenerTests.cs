...
public sealed class TlsListenerTests
{
    ...
    [Fact]
    public async Task StartAsync_should_throw_when_server_certificate_does_not_include_server_auth_eku()
    {
        using var certificateAuthority = CreateCertificateAuthority();
        using var certificateWithoutServerAuth = CreateSignedCertificate(certificateAuthority, "localhost", serverAuth: false, clientAuth: true);
        var certificatePath = CreatePkcs12File(certificateWithoutServerAuth, "password");
        ...
        try
        {
            ...
            Assert.Equal($"Failed to load TLS server certificate from '{certificatePath}'.", exception.Message);
            Assert.IsType<InvalidOperationException>(exception.InnerException);
            Assert.Equal(
                $"TLS server certificate '{certificatePath}' must include the Server Authentication EKU.",
    ...
    public async Task StartAsync_should_ingest_event_over_tls()
    {
        ...
        try
        {
            ...
            Assert.Equal(IngestionMethod.Tls, syslogEvent.EventSource.IngestionMethod);
