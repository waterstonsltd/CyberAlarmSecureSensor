...
public sealed class PatternMatcherTests
{
    ...
    [Theory]
    [InlineData("kernel: [ipv6-INP-filter-default-D] SRC=fc00::1 DST=fc00::2")]
    [InlineData("kernel: [ipv4-INP-filter-default-D] SRC=172.17.1.2 DST=8.8.8.8")]
    [InlineData("kernel: [DST-NAT-1] SRC=172.17.1.2 DST=8.8.8.8")]
    public void TryMatch_should_match_exacube_faros_firewall_pattern(string log)
    {
        // Arrange — mirrors docs/parsers/exacube-faros.json.
        var pattern = new PatternBuilder()
            .WithContainsAllRule(["kernel: [", "SRC=", "DST="])
            .WithContainsAnyRule(["[ipv4-INP-filter-default-", "[ipv6-INP-filter-default-", "[DST-NAT-"])
            .Build();
    ...
    [Theory]
    [InlineData("<14>Aug 19 05:53:04 EXAHW00002B2F3 charon-systemd[7036]: sending DPD request")]
    [InlineData("<14>Aug 19 05:53:04 EXAHW00002B2F3 charon-systemd[7036]: generating INFORMATIONAL request 6386 [ ]")]
    [InlineData("<14>Aug 19 11:10:18 EXAHW00002B2F3 netdata[4094]: time=2026-08-19T10:10:18.273+00:00 comm=netdata source=access level=info req_method=GET code=200")]
    public void TryMatch_should_match_exacube_faros_system_noise_pattern(string log)
    {
        // Arrange — mirrors docs/parsers/exacube-faros-system-noise.json.
        var pattern = new PatternBuilder()
            .WithContainsAnyRule(["charon-systemd[", "netdata["])
            .WithMustNotContainRule(["SRC=", "DST="])
            .Build();
