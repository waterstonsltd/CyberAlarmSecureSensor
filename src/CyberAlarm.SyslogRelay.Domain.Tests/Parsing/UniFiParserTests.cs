...
public sealed class UniFiParserTests
{
    ...
    private static LinuxNetfilterParserConfig Config => new()
    {
        ...
        AllowActionValues = ["A", "DNAT"],
        DenyActionValues = ["D"],
        ActionExtractRegex = @"\[[^\]\s]+-(DNAT|[A-Z])(?:-\d+)?\]",
    ...
    public static TheoryData<string, ParseResult> TestLogs =>
        new()
        {
            ...
            // EdgeRouter deny – action token without trailing numeric rule id
            {
                "<4>Aug 18 14:29:05 gw-testlab kernel: [local-default-D]IN=eth0 OUT= MAC=e0:63:da:c0:5e:70:68:4f:64:fa:cb:40:08:00 SRC=46.231.231.170 DST=82.219.67.78 LEN=60 TOS=0x00 PREC=0x00 TTL=49 ID=31092 DF PROTO=TCP SPT=37230 DPT=23 WINDOW=42340 RES=0x00 SYN URGP=0",
                new("46.231.231.170", "82.219.67.78", 37230, 23, EventProtocol.Tcp, EventAction.Deny)
            },
        };
}
