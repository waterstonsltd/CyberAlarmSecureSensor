...
public sealed class KeyValueParserTests
{
    ...
    [MemberData(nameof(SophosXgsTestLogs), MemberType = typeof(KeyValueParserTests))]
    ...
    [MemberData(nameof(CheckPointTestLogs), MemberType = typeof(KeyValueParserTests))]
    ...
    public static TheoryData<KeyValueParserConfig, string, ParseResult> SophosXgsTestLogs()
    {
        // Sophos XGS uses the same key-value format as Sophos XG (src_ip, dst_ip, etc.)
        // but device_model is "XGS..." instead of "SF..."
        var config = new KeyValueParserConfigBuilder()
            .UseRegex(true)
            .WithSourceIpKeys("src_ip")
            .WithDestinationIpKeys("dst_ip")
        ...
        return new()
        {
            // XGS3300 firewall rule - Allowed (from Pool Academy CA sample data)
            {
                config,
                "<134>device_name=\"XGS3300\" timestamp=\"2026-05-22T09:19:39+0100\" device_model=\"XGS3300\" device_serial_id=\"X33008PPYQQPJFE\" log_id=\"010101600001\" log_type=\"Firewall\" log_component=\"Firewall Rule\" log_subtype=\"Allowed\" log_version=1 severity=\"Information\" fw_rule_id=\"4\" fw_rule_name=\"Allow Unifi\" fw_rule_section=\"Local rule\" nat_rule_id=\"2\" nat_rule_name=\"Default SNAT IPv4\" fw_rule_type=\"USER\" gw_id_request=3 gw_name_request=\"Zen Cisco router WAN\" ether_type=\"Unknown (0x0000)\" in_interface=\"Port1\" out_interface=\"Port4\" src_mac=\"38:21:C7:5A:7A:62\" dst_mac=\"7C:5A:1C:CB:A9:4F\" src_ip=\"10.0.86.223\" src_country=\"R1\" dst_ip=\"17.250.81.26\" dst_country=\"GBR\" protocol=\"UDP\" src_port=60248 dst_port=443 src_trans_ip=\"51.148.45.81\" src_zone_type=\"LAN\" src_zone=\"LAN\" dst_zone_type=\"WAN\" dst_zone=\"WAN\" con_event=\"Start\" con_id=\"3039717960\" hb_status=\"No Heartbeat\" app_resolved_by=\"Signature\" app_is_cloud=\"FALSE\" qualifier=\"New\" in_display_interface=\"Port1\" out_display_interface=\"Port4 - Zen - Data WAN\" log_occurrence=\"1\"",
                new("10.0.86.223", "17.250.81.26", 60248, 443, EventProtocol.Udp, EventAction.Allow)
            },
            // XGS3300 firewall rule - TCP Allowed
            {
                config,
                "<134>device_name=\"XGS3300\" timestamp=\"2026-05-22T09:19:39+0100\" device_model=\"XGS3300\" device_serial_id=\"X33008PPYQQPJFE\" log_id=\"010101600001\" log_type=\"Firewall\" log_component=\"Firewall Rule\" log_subtype=\"Allowed\" log_version=1 severity=\"Information\" fw_rule_id=\"2\" fw_rule_name=\"Default - Base LAN <> WAN & IPS\" fw_rule_section=\"Local rule\" nat_rule_id=\"2\" nat_rule_name=\"Default SNAT IPv4\" fw_rule_type=\"USER\" gw_id_request=3 gw_name_request=\"Zen Cisco router WAN\" ether_type=\"Unknown (0x0000)\" in_interface=\"Port1\" out_interface=\"Port4\" src_mac=\"38:21:C7:5A:7A:62\" dst_mac=\"7C:5A:1C:CB:A9:4F\" src_ip=\"10.0.86.223\" src_country=\"R1\" dst_ip=\"17.250.81.26\" dst_country=\"GBR\" protocol=\"TCP\" src_port=64341 dst_port=443 src_trans_ip=\"51.148.45.81\" src_zone_type=\"LAN\" src_zone=\"LAN\" dst_zone_type=\"WAN\" dst_zone=\"WAN\" con_event=\"Start\" con_id=\"3898250774\" hb_status=\"No Heartbeat\" app_resolved_by=\"Signature\" app_is_cloud=\"FALSE\" qualifier=\"New\" in_display_interface=\"Port1\" out_display_interface=\"Port4 - Zen - Data WAN\" log_occurrence=\"1\"",
                new("10.0.86.223", "17.250.81.26", 64341, 443, EventProtocol.Tcp, EventAction.Allow)
            },
            // XGS - Denied action
            {
                config,
                "<134>device_name=\"XGS3300\" device_model=\"XGS3300\" log_type=\"Firewall\" log_component=\"Firewall Rule\" log_subtype=\"Denied\" src_ip=\"192.168.1.100\" dst_ip=\"8.8.8.8\" protocol=\"TCP\" src_port=54321 dst_port=80",
                new("192.168.1.100", "8.8.8.8", 54321, 80, EventProtocol.Tcp, EventAction.Deny)
            },
            // XGS - Dropped action
            {
                config,
                "<134>device_name=\"XGS3300\" device_model=\"XGS3300\" log_type=\"Firewall\" log_component=\"Firewall Rule\" log_subtype=\"Dropped\" src_ip=\"10.0.0.50\" dst_ip=\"203.0.113.1\" protocol=\"UDP\" src_port=12345 dst_port=53",
                new("10.0.0.50", "203.0.113.1", 12345, 53, EventProtocol.Udp, EventAction.Drop)
            },
            // XGS - ICMP (no ports)
            {
                config,
                "<134>device_name=\"XGS3300\" device_model=\"XGS3300\" log_type=\"Firewall\" log_component=\"Firewall Rule\" log_subtype=\"Allowed\" src_ip=\"10.0.1.5\" dst_ip=\"10.0.1.1\" protocol=\"ICMP\"",
                new("10.0.1.5", "10.0.1.1", null, null, EventProtocol.Icmp, EventAction.Allow)
            },
        };
    }
    ...
    public static TheoryData<KeyValueParserConfig, string, ParseResult> CheckPointTestLogs()
    {
        // Mirrors the real Check Point pattern from docs/parsers/checkpoint.json
        var config = new KeyValueParserConfigBuilder()
            .UseRegex(true)
            .WithRegexPatternOverride("(\\w+):\\s*(?:\"([^\"]*)\"|([^;\\]\\s]+))")
            .WithSourceIpKeys("src")
            .WithDestinationIpKeys("dst")
            .WithSourcePortKeys("s_port")
            .WithDestinationPortKeys("service")
            .WithProtocolKeys("proto")
            .WithActionKeys("action", "rule_action")
            .WithActionValues(
                allowValues: ["Accept"],
                denyValues: ["Prevent", "Reject"],
                dropValues: ["Drop"])
            .WithDurationKeys(isSeconds: true, "duration")
            .WithTotalBytesKeys("bytes")
            .WithOptional(sourcePort: true, destinationPort: true)
            .Build();

        return new()
        {
            // UDP Accept, sourced from a real relay capture (RawData)
            {
                config,
                "<134>1 2026-09-14T09:08:59Z flscpmgmtaz001 CheckPoint 11918 - [action:\"Accept\"; conn_direction:\"Internal\"; ifdir:\"inbound\"; ifname:\"eth1\"; origin:\"172.21.12.4\"; dst:\"172.19.1.20\"; rule_action:\"Accept\"; rule_name:\"Allw HTTPS for ZScaler\"; proto:\"17\"; s_port:\"46548\"; service:\"53\"; service_id:\"domain-udp\"; src:\"172.22.1.21\"]",
                new("172.22.1.21", "172.19.1.20", 46548, 53, EventProtocol.Udp, EventAction.Allow)
            },
            // TCP Drop (Stealth Rule), sourced from a real relay capture (RawData)
            {
                config,
                "<134>1 2026-09-14T09:08:56Z flscpmgmtaz001 CheckPoint 11918 - [action:\"Drop\"; ifdir:\"inbound\"; ifname:\"eth0\"; origin:\"172.21.12.5\"; dst:\"172.21.12.5\"; inzone:\"External\"; rule_action:\"Drop\"; rule_name:\"Stealth Rule\"; outzone:\"Local\"; proto:\"6\"; s_port:\"25019\"; service:\"21\"; service_id:\"ftp\"; src:\"5.39.222.138\"]",
                new("5.39.222.138", "172.21.12.5", 25019, 21, EventProtocol.Tcp, EventAction.Drop)
            },
            // ICMP Accept: no s_port/service keys present
            {
                config,
                "<134>1 2026-09-14T09:09:00Z flscpmgmtaz001 CheckPoint 11918 - [action:\"Accept\"; ifdir:\"inbound\"; dst:\"172.19.1.20\"; icmp:\"Echo Request\"; icmp_code:\"0\"; icmp_type:\"8\"; rule_action:\"Accept\"; rule_name:\"Allow NC2 Subnets to Azure Domain Controllers\"; proto:\"1\"; service_id:\"icmp-proto\"; src:\"172.27.6.31\"]",
                new("172.27.6.31", "172.19.1.20", null, null, EventProtocol.Icmp, EventAction.Allow)
            },
            // Application Control log with duration/bytes, and only rule_action present as an actionable key when action differs
            {
                config,
                "<134>1 2026-09-14T07:08:19Z flscpmgmtaz001 CheckPoint 11918 - [action:\"Redirect\"; dst:\"52.123.242.85\"; duration:\"7260\"; rule_action:\"Inline\"; rule_action:\"Redirect\"; rule_name:\"Internet Access\"; proto:\"6\"; service:\"443\"; src:\"172.23.6.13\"; bytes:\"307065\"]",
                new("172.23.6.13", "52.123.242.85", null, 443, EventProtocol.Tcp, EventAction.Unknown, TimeSpan.FromSeconds(7260), 307065)
            },
        };
    }
}
