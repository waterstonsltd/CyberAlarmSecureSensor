...
public sealed class SonicWallParserTests
{
    ...
    [Theory]
    [InlineData(@"<134>  id=sw_testsite sn=AABBCCDD1234 time=""2001-01-01 10:00:00"" fw=192.0.2.1 pri=6 c=262144 m=98 msg=""Connection Opened"" n=1000001 src=203.0.113.10:54626:X1 dst=192.0.2.1:16063:X1 proto=tcp/16063 sent=46 dpi=0 fw_action=""NA""", "203.0.113.10", 54626, "192.0.2.1", 16063, EventProtocol.Tcp, EventAction.Unknown)]
    [InlineData(@"<134>  id=SonicWall sn=AABBCCDD5678 time=""2001-01-01 11:00:00"" fw=192.0.2.2 pri=6 c=1024 m=537 msg=""Connection Closed"" app=49169 appName='General DNS' n=1000002 src=198.51.100.10:37975:X2 dst=192.0.2.100:53:X1 srcMac=02:00:00:00:00:01 dstMac=02:00:00:00:00:02 proto=udp/dns sent=224 rcvd=395 spkt=3 rpkt=3 dpi=0 cdur=194000 rule=""Default Access Rule"" fw_action=""NA""", "198.51.100.10", 37975, "192.0.2.100", 53, EventProtocol.Udp, EventAction.Unknown)]
    [InlineData(@"<129>  id=firewall sn=AABBCCDD9012 time=""2001-01-01 12:00:00"" fw=192.0.2.3 pri=1 c=0 gcat=6 m=1099 msg=""DNS rebind attack blocked"" srcMac=02:00:00:00:00:03 src=203.0.113.20:53:X1 srcZone=LAN natSrc=203.0.113.20:53 dstMac=02:00:00:00:00:04 dst=198.51.100.20:52293:X2 dstZone=WAN natDst=192.0.2.3:25639 proto=udp/dns rcvd=95 rule=""Test DNS Rule"" app=2 note=""FQDN=test.example.com; HOST=127.0.0.1"" n=1000003 fw_action=""drop""", "203.0.113.20", 53, "198.51.100.20", 52293, EventProtocol.Udp, EventAction.Drop)]
    public void Parse_should_succeed_for_nsa2700_and_tz380_logs(string log, string expectedSrcIp, int expectedSrcPort, string expectedDstIp, int expectedDstPort, EventProtocol expectedProtocol, EventAction expectedAction)
    {
        _unitUnderTest.Initialise(Config);
        ...
        Assert.True(result.IsSuccess);
        var parsed = result.Value;
        Assert.Equal(expectedSrcIp, parsed.SourceIp);
        Assert.Equal(expectedSrcPort, parsed.SourcePort);
        Assert.Equal(expectedDstIp, parsed.DestinationIp);
        Assert.Equal(expectedDstPort, parsed.DestinationPort);
