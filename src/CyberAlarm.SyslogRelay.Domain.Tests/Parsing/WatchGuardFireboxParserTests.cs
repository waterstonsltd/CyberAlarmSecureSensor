...
public sealed class WatchGuardFireboxParserTests
{
    ...
    public static TheoryData<string, ParseResult> TestLogs =>
        new()
        {
            ...
            // Bug 1757: multi-word destination interface name (e.g. "BT 1GB Internet")
            {
                @"<140>Jun 01 10:00:00 TestFirebox02 (2026-06-01T10:00:00) firewall: msg_id=""3000-0148"" Allow Internal BT 1GB Internet 52 tcp 20 126 192.0.2.1 203.0.113.1 62315 443 offset 8 S 123456789 win 61690  geo_dst=""NLD""  (Allow Web Browsing Out-00)",
                new("192.0.2.1", "203.0.113.1", 62315, 443, EventProtocol.Tcp, EventAction.Allow)
            },
            // Bug 1757: multi-word source interface name (e.g. "BT 1GB Internet") with geo + bytes fields
            {
                @"<140>Jun 01 10:00:01 TestFirebox02 (2026-06-01T10:00:01) firewall: msg_id=""3000-0148"" Deny BT 1GB Internet Firebox 40 tcp 20 243 203.0.113.2 192.0.2.99 60021 61616 offset 5 S 987654321 win 4  geo_src=""RUS""  geo_dst=""GBR"" flags=""SR"" duration=""0 seconds"" sent_pkts=""1"" rcvd_pkts=""0"" sent_bytes=""40"" rcvd_bytes=""0""  (Unhandled External Packet-00)",
                new("203.0.113.2", "192.0.2.99", 60021, 61616, EventProtocol.Tcp, EventAction.Deny, null, 40)
            },
            // Bug 1757: multi-word source and destination interface names
            {
                @"<142>Jun 01 10:00:02 TestFirebox02 (2026-06-01T10:00:02) firewall: msg_id=""3000-0151"" Allow BT Primary WAN Corporate LAN udp 192.0.2.2 203.0.113.3 50001 53 flags=""SD"" duration=""1 seconds"" sent_pkts=""1"" rcvd_pkts=""1"" sent_bytes=""60"" rcvd_bytes=""80""",
                new("192.0.2.2", "203.0.113.3", 50001, 53, EventProtocol.Udp, EventAction.Allow, null, 140)
            },
        };
}
