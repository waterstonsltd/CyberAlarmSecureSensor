    ...
    : PipelineStageBase<ValidationStageOutput, bool>(services, logger), IDisposable
{
    ...
    // In-memory accumulation buffer — only ever accessed by the single processing loop worker,
    ...
    protected override async Task<bool> ProcessMessageAsync(ValidationStageOutput input, CancellationToken cancellationToken)
    {
        ...
        if (_buffer.Count >= _options.PersistenceBufferSize)
        {
    ...
    private async Task EnqueueFlush(FlushReason reason, CancellationToken cancellationToken)
    {
        ...
        // Swap out the buffer so the processing loop can immediately continue while
        // the writer task persists the previous batch independently.
        var batch = new List<ParsedEvent>(_buffer);
        _buffer.Clear();
