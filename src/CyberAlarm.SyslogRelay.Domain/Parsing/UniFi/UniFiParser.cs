...
// Parses UniFi/UDM/EdgeRouter firewall kernel log messages of the form:
//   [ZONE-A-RULENUM] ... SRC=x DST=x ... PROTO=TCP SPT=x DPT=x ...
//   [local-default-D] ... SRC=x DST=x ... PROTO=TCP SPT=x DPT=x ...
...
internal sealed partial class UniFiParser : KeyValueParserBase<ParserConfig>
{
    ...
    // Matches UniFi/EdgeRouter rule-chain brackets where the action token appears before
    // an optional numeric rule id, e.g. [WAN_IN-D-100], [PREROUTING-DNAT-5], [local-default-D].
    // Group 1 captures the action token.
    [GeneratedRegex(@"\[[^\]\s]+-(DNAT|[A-Z])(?:-\d+)?\]")]
