...
{
    [GeneratedRegex(@"(?:<\d+>:\s*)?%ASA-(?:\w+-)?(?<severity>\d)-(?<message_id>\d{6}):\s+(?<message_content>.+)$", RegexOptions.Compiled)]
    ...
    [GeneratedRegex(@"^Deny\s+(?<protocol>\w+)\s+src\s+(?:[^:\s]+):(?<src_ip>\d{1,3}(?:\.\d{1,3}){3})(?:/(?<src_port>\d+))?\s+dst\s+(?:[^:\s]+):(?<dst_ip>\d{1,3}(?:\.\d{1,3}){3})(?:/(?<dst_port>\d+))?(?:\s+\([^)]*\))?\s+by\s+access-group\b.*$", RegexOptions.Compiled)]
    ...
    [GeneratedRegex(@"^Built\s+(?<direction>in|out)bound\s+(?<protocol>TCP)\s+connection\s+\d+\s+for\s+\w+:(?<src_ip>\d{1,3}(?:\.\d{1,3}){3})/(?<src_port>\d+)\s+\([^)]+\)\s+to\s+\w+:(?<dst_ip>\d{1,3}(?:\.\d{1,3}){3})/(?<dst_port>\d+)", RegexOptions.Compiled)]
    public static partial Regex Regex_302013();

    [GeneratedRegex(@"^Built\s+(?<direction>in|out)bound\s+(?<protocol>UDP)\s+connection\s+\d+\s+for\s+\w+:(?<src_ip>\d{1,3}(?:\.\d{1,3}){3})/(?<src_port>\d+)\s+\([^)]+\)\s+to\s+\w+:(?<dst_ip>\d{1,3}(?:\.\d{1,3}){3})/(?<dst_port>\d+)", RegexOptions.Compiled)]
    public static partial Regex Regex_302015();

    [GeneratedRegex(@"^Teardown\s+(?<protocol>UDP)\s+connection\s+\d+\s+for\s+(?<first_iface>\w+):(?<src_ip>\d{1,3}(?:\.\d{1,3}){3})/(?<src_port>\d+)\s+to\s+\w+:(?<dst_ip>\d{1,3}(?:\.\d{1,3}){3})/(?<dst_port>\d+)", RegexOptions.Compiled)]
    public static partial Regex Regex_302016();

    [GeneratedRegex(@"^Teardown\s+(?<protocol>TCP)\s+connection\s+\d+\s+for\s+(?<first_iface>\w+):(?<src_ip>\d{1,3}(?:\.\d{1,3}){3})/(?<src_port>\d+)\s+to\s+\w+:(?<dst_ip>\d{1,3}(?:\.\d{1,3}){3})/(?<dst_port>\d+)", RegexOptions.Compiled)]
    public static partial Regex Regex_302014_303002();

    [GeneratedRegex(@"^(?:Deny\s+(?<protocol>icmp)\s+src\s+(?:\w+):(?<src_ip>\d{1,3}(?:\.\d{1,3}){3})\s+dst\s+(?:\w+):(?<dst_ip>\d{1,3}(?:\.\d{1,3}){3})|Denied\s+ICMP\s+type=\d+,\s+code=\d+\s+from\s+(?<src_ip>\d{1,3}(?:\.\d{1,3}){3})\s+on\s+interface\s+\w+)", RegexOptions.Compiled | RegexOptions.IgnoreCase)]
