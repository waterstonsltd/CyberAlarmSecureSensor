    ...
    ILogger<StatusService> logger) : IStatusService
{
    ...
    private async Task<Result<RelayStatus>> RefreshStatus(CancellationToken cancellationToken)
    {
        ...
        if (result.HasError<StatusNotModifiedWarning>())
        {
            _logger.LogInformation("Skipping status refresh: {ErrorMessage} Attempting to read status from file.", result.ErrorMessage);
        }
        else
        {
            _logger.LogWarning("Failed to refresh status: {ErrorMessage} Attempting to read status from file.", result.ErrorMessage);
        }
