...
{
    private readonly (Pattern Pattern, PatternMatchResult Result)[] _patterns;
    ...
    public PatternMatcher(List<Pattern> patterns, int scanLength)
    {
        ...
        // Sort by priority (highest first) and pre-allocate results so TryMatch is allocation-free on the hot path
        _patterns = patterns
            .OrderByDescending(p => p.Priority)
            .Select(p => (p, new PatternMatchResult(p.Name, p.Parser, p.IgnoreIfContaining)))
            .ToArray();
    ...
    {
        foreach (var (pattern, result) in _patterns)
        {
