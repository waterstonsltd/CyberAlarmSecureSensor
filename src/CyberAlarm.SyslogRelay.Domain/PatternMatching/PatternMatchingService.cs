    ...
    ILogger<PatternMatchingService> logger) : IPatternMatchingService, IDisposable
{
    ...
    private async Task<List<Pattern>> GetPatterns(CancellationToken cancellationToken)
    {
        ...
        var status = result.IsSuccess
            ? result.Value
            : await _statusService.GetStatusAsync(cancellationToken);
    ...
    private List<Pattern> HydrateWithParsers(StatusPattern[] patterns)
    {
        ...
        {
            var parser = _parserFactory.Create(pattern.ParserClass, pattern.ParserConfig);
            if (parser is null)
            {
            ...
            parserPatterns.Add(new(pattern.Name, parser, pattern.Priority, pattern.Rules, pattern.IgnoreIfContaining));
        }
