...
public static class ServiceCollectionExtensions
{
    ...
    {
        services.AddHealthCheckFor([
            nameof(InitialisationService),
            nameof(PatternMatchingStage),
            nameof(ParsingStage),
            nameof(ValidationStage),
            nameof(BufferedPersistenceStage),
            nameof(FileWatcher),
            nameof(TcpListener),
            nameof(TlsListener),
            nameof(UdpListener),
        ...
        services.AddTransient<PipelineStageServices>();
        ...
        services.AddScoped<InitialisationService>()
            ...
            .AddTransient<IStartupActivity, InitialiseHealthCheckActivity>()
            ...
            .AddTransient<IStartupActivity, InitialiseUploadMetricsActivity>();
        ...
        services.AddSingleton<UploadPipelineServices>();
        services.AddSingleton<ISecureFtpClientFactory, SecureFtpClientFactory>();
        if (!configuration.GetValue<bool>(nameof(RelayOptions.ForceHttpsUpload)))
        {
            services.AddSingleton<IUploadChannelFactory, SftpUploadChannelFactory>();
        }

        services.AddSingleton<IUploadChannelFactory, HttpsSasUploadChannelFactory>();
        services.AddHttpClient(nameof(SecureUploader));
        ...
        services.AddSingleton<TlsListener>();
        ...
        services.AddSingleton<IParserFactory, ParserFactory>()
            .AddParser<CefParser>()
            .AddParser<CiscoAsaParser>()
            .AddParser<CiscoFirepowerAsaStyleParser>()
            .AddParser<BarracudaCgfKvpParser>()
            .AddParser<BarracudaCgfParser>()
            .AddParser<BarracudaCgfThreatParser>()
            .AddParser<DraytekParser>()
            .AddParser<MerakiParser>()
            .AddParser<PaloAltoParser>()
            .AddParser<PfSenseParser>()
            .AddParser<MikrotikParser>()
            .AddParser<LinuxNetfilterParser>()
            .AddParser<JsonParser>()
            .AddParser<KeyValueParser>()
            .AddParser<SmoothwallParser>()
            .AddParser<WatchGuardDimensionParser>()
            .AddParser<WatchGuardFireboxParser>()
            .AddParser<SonicWallParser>()
            .AddParser<UniFiParser>()
            .AddParser<UntangleParser>()
            .AddParser<NullParser>();
        services.AddSingleton(provider =>
            PipelineBuilder
                ...
                .Then(provider => provider.GetRequiredService<ValidationStage>())
        ...
        services.AddTransient<SupportBundleService>();
    ...
    public static IServiceCollection AddHealthCheckFor(this IServiceCollection services, string[] servicesToRegister)
    {
        services.AddSingleton<IHealthCheckService, HealthCheckService>();
        services.PostConfigure<HealthCheckOptions>(options => options.ServicesToRegister = servicesToRegister);

        return services;
    }

    public static IServiceCollection AddParser<TParser>(this IServiceCollection services)
        where TParser : class, IParser
    {
        services.AddKeyedTransient<IParser, TParser>(typeof(TParser).Name);
        return services;
    }
}
