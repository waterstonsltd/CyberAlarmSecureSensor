    ...
    ILogger<TcpListener> logger) : IDisposable
{
    ...
    public async Task StartAsync(Func<SyslogEvent, CancellationToken, Task> ingestAction, CancellationToken cancellationToken)
    {
        ...
        await _healthCheckService.SetHealthyAsync(nameof(TcpListener), _cts.Token);
        ...
        catch (Exception ex)
        {
            ...
            await _healthCheckService.SetUnhealthyAsync(nameof(TcpListener), cancellationToken);
        }
        finally
        {
            Dispose();
