    ...
    ILogger<UdpListener> logger) : IDisposable
{
    ...
    public async Task StartAsync(Func<SyslogEvent, CancellationToken, Task> ingestAction, CancellationToken cancellationToken)
    {
        ...
        await _healthCheckService.SetHealthyAsync(nameof(UdpListener), _cts.Token);
        ...
        catch (Exception ex)
        {
            ...
            await _healthCheckService.SetUnhealthyAsync(nameof(UdpListener), cancellationToken);
        }
        finally
        {
            Dispose();
