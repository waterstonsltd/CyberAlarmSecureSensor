    ...
    ILogger<UdpListener> logger) : IDisposable
{
    ...
    public async Task StartAsync(Func<SyslogEvent, CancellationToken, Task> ingestAction, CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation("Starting UDP listener on port {Port}.", _options.UdpPort);

        _udpClient = new();
        _udpClient.Client.Bind(new IPEndPoint(IPAddress.Any, _options.UdpPort));
        ...
        catch (Exception ex)
        {
            ...
            _applicationManager.StopApplication("UDP listener error");
        }
        finally
        {
            Dispose();
