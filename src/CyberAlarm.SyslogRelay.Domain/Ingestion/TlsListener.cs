    ...
    ILogger<TlsListener> logger) : IDisposable
{
    ...
    public async Task StartAsync(Func<SyslogEvent, CancellationToken, Task> ingestAction, CancellationToken cancellationToken)
    {
        ...
        // Allows rebind after clean restart / TIME_WAIT — safe for a single-instance Windows Service
        ...
        catch (OperationCanceledException ex)
        {
            ...
            {
                _logger.LogInformation(ex, "TLS listener was cancelled.");
            }
        }
        catch (Exception ex)
        {
            ...
            // Use CancellationToken.None — _cts.Token may already be cancelled
            await _healthToken.UnhealthyAsync(CancellationToken.None);
            _applicationManager.StopApplication("TLS listener error");
        }
        finally
        {
            Dispose();
    ...
    public void Dispose()
    {
        ...
        if (_listener != null)
        {
            ...
            {
                // Socket may already be in a bad state — safe to ignore
            }
            finally
            {
                _listener.Close();
    ...
    private async Task HandleClient(Socket clientSocket, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogDebug(ex, "[{Client}] handling cancelled.", client);
        }
        ...
        finally
        {
            ...
            {
                _logger.LogDebug(ex, "[{Client}] socket already closed while shutting down.", client);
            }
