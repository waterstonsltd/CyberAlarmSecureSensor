    ...
    IConfiguration configuration)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var ingestIssues = await FlushIfIssues(output, w => WriteIngestPipeline(w, cancellationToken));
        issueCount += ingestIssues;
        ingestHadIssues = ingestIssues > 0;
        ...
        // Network section: always show when ingest has issues (provides the fix — where to point the sender)
        if (ingestHadIssues)
        {
            if (!platform.RunningInContainer)
            {
