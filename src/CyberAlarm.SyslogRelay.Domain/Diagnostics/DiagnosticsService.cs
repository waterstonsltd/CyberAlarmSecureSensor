    ...
    IConfiguration configuration)
{
    ...
    {
        // Check if port is in use WITHOUT binding (binding causes TIME_WAIT issues
        // that can prevent the service from restarting).
        // Use IPGlobalProperties to inspect active listeners non-intrusively.
        var ipProperties = IPGlobalProperties.GetIPGlobalProperties();
        var tcpListeners = ipProperties.GetActiveTcpListeners();

        var portInUse = tcpListeners.Any(ep => ep.Port == port);
        ...
        // Port is in use. Check if it's our service (expected) or something else (conflict).
        try
        {
