...
{
    public ISecureFtpClient Create(string hostName, string userName, string privateKey, string hostFingerprint)
    {
        var privateKeyFile = new PrivateKeyFile(new MemoryStream(Encoding.UTF8.GetBytes(privateKey)));
        return new SecureFtpClient(hostName, userName, privateKeyFile, hostFingerprint);
    }
}
