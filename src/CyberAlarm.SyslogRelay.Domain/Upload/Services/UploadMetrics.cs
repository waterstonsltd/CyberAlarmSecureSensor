...
internal sealed class UploadMetrics
{
    ...
    public UploadMetrics(IMeterFactory meterFactory, IOptions<RelayOptions> relayOptions)
    {
        var meter = meterFactory.Create(MeterName);
        var buildVersion = relayOptions.Value.BuildVersion;

        // Build info — always 1, version exposed as a tag for Grafana label queries
