    ...
    ILogger<SourceGrouper> logger) : ISourceGrouper
{
    ...
    private readonly long _maxFileSizeBytes = relayOptions.Value.RawGroupedLogsMaxFileSizeBytes;
    private readonly bool _uploadRawLogs = pipelineOptions.Value.UploadRawLogs;
    private readonly List<TempFileInfo> _metaData = [];
    private int _outputFilesCreated;
    ...
    public async Task GroupLogsBySourceAsync(CancellationToken cancellationToken)
    {
                ...
                if (parsedEvents is not null)
                {
                    ...
                    foreach (var ingestionGroup in ingestionGroups)
                    {
                        ...
                        foreach (var sourceGroup in sourceGroups)
                        {
                            ...
                            var (bundleEvents, eventCounters) = GetBundleEvents(sourceGroup, _uploadRawLogs);
                            ...
                            _uploadMetrics.GroupingEventsWritten.Add(bundleEvents.Count);
                        }
                    }
        ...
        // Persist events metadata to file, merging with any existing entries from a previous run
        ...
        var existingMetaData = await _fileManager.DeserialiseFromFileAsync<Dictionary<string, EventsMetaData>>(eventsMetaDataFilePath, cancellationToken);
        if (existingMetaData is not null)
        {
            foreach (var entry in existingMetaData)
            {
                eventsMetaData.TryAdd(entry.Key, entry.Value);
            }
        }
        ...
        stopwatch.Stop();
        _uploadMetrics.GroupingFilesProcessed.Add(processedCount);
        _uploadMetrics.GroupingFilesFailed.Add(failedCount);
        _uploadMetrics.GroupingOutputFilesCreated.Add(_outputFilesCreated);
        _uploadMetrics.GroupingDuration.Record(stopwatch.Elapsed.TotalMilliseconds);
        _outputFilesCreated = 0;
        _logger.LogInformation(
    ...
    private static (List<BundleEvent> Events, EventCounters EventCounters) GetBundleEvents(IEnumerable<ParsedEvent> events, bool uploadRawLogs)
    {
        ...
        foreach (var @event in events)
        {
            ...
            switch (@event.ValidationStatus)
            {
                case ValidationStatus.UnableToPatternMatch:
                    CountAndConditionallyInclude(bundleEvents, @event, uploadRawLogs, ref unmatchedEvents);
                    break;

                case ValidationStatus.UnableToParse:
                    CountAndConditionallyInclude(bundleEvents, @event, uploadRawLogs, ref unparsedEvents);
                    break;

                case ValidationStatus.LocalOnlyEvent:
                    CountAndConditionallyInclude(bundleEvents, @event, uploadRawLogs, ref localOnlyEvents);
                    break;

                case ValidationStatus.OutboundEvent:
                    CountAndConditionallyInclude(bundleEvents, @event, uploadRawLogs, ref outboundEvents);
                    break;

                case ValidationStatus.Ignored:
                    CountAndConditionallyInclude(bundleEvents, @event, uploadRawLogs, ref ignoredEvents);
                    break;

                default:
                    TryAddBundleEvent(bundleEvents, @event, includeRawData: uploadRawLogs);
                    break;
            }
        }

        return (bundleEvents, new(totalEvents, unmatchedEvents, unparsedEvents, localOnlyEvents, outboundEvents, ignoredEvents));
    }

    private static void CountAndConditionallyInclude(List<BundleEvent> bundleEvents, ParsedEvent @event, bool uploadRawLogs, ref int counter)
    {
        counter++;
        if (uploadRawLogs)
        {
            TryAddBundleEvent(bundleEvents, @event, includeRawData: true);
        }
    }

    private static void TryAddBundleEvent(List<BundleEvent> bundleEvents, ParsedEvent @event, bool includeRawData)
    {
        if (@event.ParseResult is null && (string.IsNullOrEmpty(@event.RawData) || !includeRawData))
        {
            return;
        }

        var rawData = includeRawData ? @event.RawData : null;
        bundleEvents.Add(new(
            @event.Timestamp,
            rawData,
            @event.PatternName,
            @event.ParseResult,
            includeRawData ? @event.ValidationStatus : null));
    }
        ...
        string source)
    {
        ...
        public EventsMetaData TotEventsMetaData() =>
            new(
                ...
                EventCounters.OutboundEvents,
                EventCounters.IgnoredEvents);
    }
        ...
        int ignoredEvents)
    {
        ...
        public int OutboundEvents { get; private set; } = outboundEvents;

        public int IgnoredEvents { get; private set; } = ignoredEvents;
        ...
        public void Update(EventCounters eventCounters)
        {
            ...
            OutboundEvents += eventCounters.OutboundEvents;
            IgnoredEvents += eventCounters.IgnoredEvents;
        }
    }
}
