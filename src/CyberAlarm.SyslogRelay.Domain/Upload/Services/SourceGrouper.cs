    ...
    ILogger<SourceGrouper> logger) : ISourceGrouper
{
        ...
        foreach (var @event in events)
        {
            ...
            switch (@event.ValidationStatus)
            {
                case ValidationStatus.UnableToPatternMatch:
                    ...
                    if (uploadRawLogs)
                    {
                        TryAddBundleEvent(bundleEvents, @event);
                    }
    ...
    private static void TryAddBundleEvent(List<BundleEvent> bundleEvents, ParsedEvent @event)
    {
        if (@event.ParseResult is null && string.IsNullOrEmpty(@event.RawData))
        {
            return;
        }
