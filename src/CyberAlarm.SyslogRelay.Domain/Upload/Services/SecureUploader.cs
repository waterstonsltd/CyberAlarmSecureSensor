    ...
    ILogger<SecureUploader> logger) : ISecureUploader
{
    ...
    public async Task UploadFilesAsync(CancellationToken cancellationToken)
    {
        ...
        var hostFingerprints = new List<string> { status.HostFingerprint };
        if (status.SecondaryHostFingerprint is not null)
        {
            hostFingerprints.Add(status.SecondaryHostFingerprint);
        }

        using var secureFtpClient = secureFtpClientFactory.Create(targetServer, userName, privateKey, hostFingerprints);
        ...
        foreach (var file in filesToUpload)
        {
            ...
            if (result.TimeoutTargetServer is not null)
            {
                _logger.LogError(
                    "Upload failed: could not connect to {TargetServer} on TCP port 22 within the timeout period. "
                    + "This is typically caused by a firewall blocking outbound port 22. "
                    + "{Remaining} file(s) will not be attempted this cycle and will be retried automatically on the next cycle. "
                    + "See the troubleshooting guide (Section 6: Test outbound upload connectivity) for steps to diagnose and fix the connection.",
                    result.TimeoutTargetServer,
                    result.RemainingFiles);
            }

            uploadedCount += result.UploadedDelta;
            failedCount += result.FailedDelta;

            if (result.StopUploadCycle)
            {
                break;
            }
    ...
    private async Task<UploadAttemptResult> UploadFileAsync(
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            await UploadSingleFileAsync(secureFtpClient, file, inputFolder, cancellationToken);

            _fileManager.Delete(file);
            _uploadMetrics.FilesUploaded.Add(1);
            _logger.LogInformation("{FileName} successfully uploaded", file);
            return new(UploadedDelta: 1, FailedDelta: 0, StopUploadCycle: false);
        }
        catch (Exception ex) when (ex is SshAuthenticationException or SshConnectionException)
        {
            ...
            _uploadMetrics.FilesFailed.Add(1);

            await StopApplication(cancellationToken);
            return new(UploadedDelta: 0, FailedDelta: 1, StopUploadCycle: true);
        }
        catch (SshOperationTimeoutException ex)
        {
            _uploadMetrics.FilesFailed.Add(1);
            var remaining = totalFiles - uploadedCount - 1;
            _ = ex;
            return new(
                UploadedDelta: 0,
                FailedDelta: 1,
                StopUploadCycle: true,
                TimeoutTargetServer: targetServer,
        ...
        catch (Exception ex)
        {
            ...
            _uploadMetrics.FilesFailed.Add(1);
            return new(UploadedDelta: 0, FailedDelta: 1, StopUploadCycle: false);
        }
    }
    ...
    private sealed record UploadAttemptResult(
        int UploadedDelta,
        int FailedDelta,
        bool StopUploadCycle,
        string? TimeoutTargetServer = null,
