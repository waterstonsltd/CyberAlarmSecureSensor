...
public sealed class EventBundlerService : IEventBundlerService
{
    ...
    public async Task<EventBundle> BundleAsync(Stream outputBundle, Stream data, string relayId, string buildVersion, EventsMetaData eventsMetaData, Platform platform, byte[] syslogRelayPrivateKey, byte[] serverPublicKey, BundleOptions options, CancellationToken cancellationToken)
    {
        ...
        Encryption encryptionMetaData = await _eventPacker.CompressAndEncrypt(serverPublicKey, outputBundle, data, options.CompressionAlgorithm, options.SymmetricEncryptionAlgorithm, options.AsymmetricEncryptionAlgorithm, cancellationToken);

        byte[] nonce = RandomNumberGenerator.GetBytes(32);

        Document document = new Document(_BundlerVersion, metaData, server, encryptionMetaData, eventsMetaData, DateTime.UtcNow, nonce);
