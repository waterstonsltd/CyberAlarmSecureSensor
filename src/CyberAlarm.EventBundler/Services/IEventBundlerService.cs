...
public interface IEventBundlerService
{
    ...
    Task<EventBundle> BundleAsync(Stream outputBundle, Stream data, Stream bufferStream, string relayId, string buildVersion, EventsMetaData eventsMetaData, Platform platform, byte[] syslogRelayPrivateKey, byte[] serverPublicKey, BundleOptions options, CancellationToken cancellationToken);
