...
#
# Update logic:
#   1. Fetch hashes.json from the CyberAlarm portal and use jq to extract the authoritative compose SHA256
...
#   4. If the portal is unreachable, leave the compose file untouched and proceed
#   5. Verify the container image signature with Cosign if available
#   6. Pull the latest container image and restart the service only if the image changed
...
# ---------------------------------------------------------------------------
# Step 3: Verify image signature with Cosign (if available)
# ---------------------------------------------------------------------------
# Extract all image references from the compose file so we verify exactly what
# will be pulled — not a hardcoded tag that may differ from the compose file.
...
# ---------------------------------------------------------------------------
# Step 4: Pull latest image and restart if updated
# ---------------------------------------------------------------------------
cd "$INSTALL_DIR"
