...
function Invoke-SensorUninstall {
    ...
    Write-Host 'Uninstalling CyberAlarm Secure Sensor ...'
...
function Invoke-DeploymentToolUninstall {
    ...
    Write-Host 'Uninstalling CyberAlarm Secure Sensor Deployment Tool ...'
...
function Invoke-BootstrapMsiUninstall {
    ...
    Write-Host 'Uninstalling CyberAlarm Secure Sensor MSI package ...'
...
$MsiFileName        = "CyberAlarmSecureSensor-win.msi"
...
if ($Uninstall) {
    ...
    elseif ($installedSensor -and $installedSensor.Version) {
        ...
        try {
            Write-Host "Downloading CyberAlarm Secure Sensor MSI $installedVersion for uninstall..."
    ...
    Write-Host 'CyberAlarm Secure Sensor uninstall completed.'
    return
}
...
if (-not $serviceExists -and -not $installedDeploymentTool -and -not $installedSensor) {
    Write-Host "No existing installation found - installing version $approvedVersion."
    $shouldInstall = $true
}
elseif ($installedDeploymentTool) {
    ...
    if ($installedDeploymentTool.Version -and ([Version]$approvedVersion -gt $installedDeploymentTool.Version)) {
        Write-Host "Upgrading Deployment Tool from $installedVersionText to $approvedVersion."
        $shouldInstall = $true
    }
    else {
        Write-Host "Deployment Tool is already installed and up to date (version $installedVersionText) - skipping MSI installation."
...
elseif ($installedSensor) {
    ...
    if ($installedSensor.Version -and ([Version]$approvedVersion -gt $installedSensor.Version)) {
        Write-Host "Upgrading installed sensor from $installedVersionText to $approvedVersion."
        $shouldInstall = $true
    }
    else {
        Write-Host "Sensor is already installed and up to date (version $installedVersionText) - updating configuration only."
    }
}
else {
