...
function Get-InstalledDeploymentTool {
    ...
    if ($displayVersion) {
        try {
            ...
            Write-Verbose "Could not parse DisplayVersion '$displayVersion' as a version number: $_"
            $parsedVersion = $null
        }
    }
...
function Invoke-SensorUninstall {
    ...
    Write-Output "Stopping CyberAlarm Secure Sensor service ..."
    $null = & sc.exe stop "$ServiceName"

    $svc = Get-Service -Name $ServiceName -ErrorAction SilentlyContinue
    if ($svc) {
        try { $svc.WaitForStatus('Stopped', [TimeSpan]::FromSeconds(15)) } catch { Write-Verbose "Timed out waiting for service '$ServiceName' to stop: $_" }
    }

    Write-Output "Deleting CyberAlarm Secure Sensor service ..."
    $null = & sc.exe delete "$ServiceName"

    if (Test-Path $InstallDir) {
        Write-Output "Removing install directory: $InstallDir"
...
if ($Uninstall) {
    if ($installedDeploymentTool -and $installedDeploymentTool.ProductCode) {
        Invoke-SensorUninstall -ServiceName $ServiceName -InstallDir $VelopackInstallDir
...
if ($Reinstall) {
    ...
    $existingWindowsUpdate = $existingConfig.WindowsUpdate

    Write-Output "Reinstalling using existing configuration (token, environment, and update settings preserved from $ConfigPath)."
}
...
$windowsUpdateConfig = if ($existingWindowsUpdate) {
    [ordered]@{
        Enabled            = if ($null -ne $existingWindowsUpdate.Enabled) { [bool]$existingWindowsUpdate.Enabled } else { $true }
        CheckIntervalHours = if ($null -ne $existingWindowsUpdate.CheckIntervalHours) { [int]$existingWindowsUpdate.CheckIntervalHours } else { 1 }
        RepositoryUrl      = if (-not [string]::IsNullOrWhiteSpace($existingWindowsUpdate.RepositoryUrl)) { $existingWindowsUpdate.RepositoryUrl } else { "https://github.com/$GitHubRepo" }
        AllowPreRelease    = if ($null -ne $existingWindowsUpdate.AllowPreRelease) { [bool]$existingWindowsUpdate.AllowPreRelease } else { [bool]$PreRelease }
    }
} else {
    [ordered]@{
        Enabled            = $true
