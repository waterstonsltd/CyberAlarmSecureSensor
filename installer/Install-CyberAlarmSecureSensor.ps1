...
.PARAMETER Uninstall
    Removes the installed CyberAlarm Secure Sensor application and then uninstalls
    the Deployment Tool MSI wrapper. This is the recommended way to fully remove
    a Windows installation created by this script.
...
if ($Uninstall) {
    ...
    if ($installedDeploymentTool -and $installedDeploymentTool.ProductCode) {
        ...
        Write-Warning 'The Deployment Tool MSI registration was not found, so there was nothing to uninstall at the MSI layer.'
    }
