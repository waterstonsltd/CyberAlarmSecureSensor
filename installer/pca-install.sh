...
readonly INSTALLER_VERSION="1.0.114"
...
readonly COMPOSE_SHA256="e0ffd098e1d038bbab4fd6362ebba9061707824fcc0eb1b0bd23401a84eab136"
...
print_banner() {
    printf '%b\n' "${BLUE}"
    print_separator
    printf '%s\n' "CyberAlarm PCA Installer v${INSTALLER_VERSION}"
    print_separator
    printf '%b\n' "${NC}"
    return 0
}
...
parse_arguments() {
    ...
    for argument in "$@"; do
        case "${argument}" in
            ...
            -y|--yes)
                ...
                ;;
            --env=*)
                PARSED_ENVIRONMENT="${argument#--env=}"
                ;;
    ...
    if [[ -n "${PARSED_ENVIRONMENT}" && "${PARSED_ENVIRONMENT}" != "dev" && "${PARSED_ENVIRONMENT}" != "uat" ]]; then
        die "Unsupported --env value '${PARSED_ENVIRONMENT}'. Use 'dev' or 'uat'."
    fi
...
check_port_514() {
    ...
    if [[ "${port_in_use}" == true ]]; then
        # Allow re-runs: if the port is held by our own container, docker compose
        # up -d will replace it gracefully — no need to block.
        if docker inspect --format '{{.State.Running}}' syslog-relay 2>/dev/null | grep -q '^true$'; then
            print_info "Port 514 is in use by the existing syslog-relay container; it will be replaced."
            return 0
        fi
    ...
    while [[ "${elapsed}" -lt "${max_wait}" ]]; do
        ...
        case "${health_status}" in
            ...
            none|"")
                ...
                ;;
            starting)
                print_info "Container still initializing..."
                ;;
            *)
...
verify_image_signature() {
    ...
    if ! command -v cosign >/dev/null 2>&1; then
        print_info "Cosign not installed — skipping optional image signature verification. Install cosign to verify image integrity before deployment."
        return 0
    fi
...
main() {
    ...
    case "${PARSED_ENVIRONMENT}" in
        dev)
            api_base_url="https://dev-api.cyberalarm.police.uk"
            print_info "Environment: dev (${api_base_url})"
            ;;
        uat)
            api_base_url="https://uat-api.cyberalarm.police.uk"
            print_info "Environment: uat (${api_base_url})"
            ;;
        *)
            ;;
    esac
    ...
    create_directories
    download_compose_file
    create_env_file "${registration_token}" "${api_base_url}"
