...
# SHA256 hash of the expected docker-compose.yaml — replaced by CI pipeline at release time
...
# Check port 514 availability - warn but do not take action
check_port_514() {
...
# Create directory structure with correct ownership for container user
create_directories() {
...
# Download and verify docker-compose.yml from GitHub
download_compose_file() {
    print_info "Downloading Docker Compose configuration from GitHub..."

    local tmp_file="$COMPOSE_FILE.tmp"

    if ! curl -fsSL "$COMPOSE_URL" -o "$tmp_file"; then
        print_error "Failed to download Docker Compose file from $COMPOSE_URL"
        rm -f "$tmp_file"
        exit 1
    fi

    print_info "Verifying Docker Compose file integrity..."

    local actual_hash
    actual_hash=$(sha256sum "$tmp_file" | cut -d' ' -f1)

    if [ "$actual_hash" != "$COMPOSE_SHA256" ]; then
        print_error "════════════════════════════════════════════════════════"
        print_error "Docker Compose file integrity check FAILED."
        print_error "The downloaded file does not match the expected hash."
        print_error ""
        print_error "Expected: $COMPOSE_SHA256"
    ...
    print_success "Docker Compose file downloaded and verified"
}
...
# Pulls latest compose file and images, then restarts if anything changed
...
install_cron_job() {
    ...
    # Generate a random minute offset (0-59)
    INSTALLED_CRON_MINUTE=$((RANDOM % 60))

    # Remove any existing CyberAlarm update entry, then add the new one
    (crontab -l 2>/dev/null | grep -v "$UPDATE_SCRIPT"; echo "$INSTALLED_CRON_MINUTE * * * * $UPDATE_SCRIPT >> /var/log/cyberalarm-update.log 2>&1") | crontab -
...
verify_image_signature() {
    ...
    if cosign verify \
        ...
        ghcr.io/waterstonsltd/cyberalarm-securesensor:stable &>/dev/null; then
        print_success "✓ Image signature verified - image is authentic and untampered"
    else
        echo ""
        print_error "════════════════════════════════════════════════════════"
        print_error "Image signature verification FAILED"
        print_error ""
        print_error "This indicates the container image may have been tampered"
        print_error "with or is not from the official Waterstons repository."
        print_error ""
