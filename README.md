...
Police CyberAlarm is a free, Home Office-funded cybersecurity monitoring service that helps organisations across the UK monitor and report suspicious cyber activity. The platform is developed by [Waterstons](https://www.waterstons.com) in partnership with the National Police Chiefs' Council (NPCC) and the National Cyber Crime Team with City of London Police and aligns with [NCSC guidance](https://www.ncsc.gov.uk/) on cybersecurity monitoring.

## Features

- **Syslog collection** receives syslog data over UDP and TCP (port 514)
- **Inbound traffic analysis** by default, only metadata from inbound connections (external → internal) is uploaded; local-only traffic (internal ↔ internal) and outbound traffic (internal → external) are filtered locally and never leave your network
...
> ```
> 73cac091edb1201f78eb1d028a0277865dc1e8f02e9d73e7b3678cb698b0b243
> ```
...
curl -fsSL https://raw.githubusercontent.com/waterstonsltd/CyberAlarmSecureSensor/refs/heads/main/installer/pca-install.sh -o pca-install.sh \
  && echo "73cac091edb1201f78eb1d028a0277865dc1e8f02e9d73e7b3678cb698b0b243  pca-install.sh" | sha256sum --check \
...
> ```
> e412d2c14f5f6e76768d94f04177a134c597b71691546c12b10e04aaea41afe0
> ```
> Cross-reference this against the hash shown on the [CyberAlarm portal](https://member.cyberalarm.police.uk) before running.
...
if ((Get-FileHash .\Install-CyberAlarmSecureSensor.ps1 -Algorithm SHA256).Hash.ToLowerInvariant() -ne "e412d2c14f5f6e76768d94f04177a134c597b71691546c12b10e04aaea41afe0") {
...
If the Windows service registration is missing or broken (for example, if the sensor was run manually instead of being installed as a service), you can re-register it without supplying the token again using `-Reinstall`. The script reads the token and environment from the existing configuration file:

```powershell
powershell -ExecutionPolicy Bypass -File .\Install-CyberAlarmSecureSensor.ps1 -Reinstall
```
...
1. Download the latest `CyberAlarmSecureSensor-stable.msi` from the [GitHub Releases](https://github.com/waterstonsltd/CyberAlarmSecureSensor/releases) page.
...
The `CyberAlarmSecureSensor-stable-Setup.exe`, `.nupkg`, `RELEASES-stable`, and JSON files on the release are update assets used by Velopack. For a new Windows service installation, use `CyberAlarmSecureSensor-stable.msi`.
...
| `UploadOutboundData` | `false` | When `true`, outbound traffic (internal → external) is uploaded alongside inbound traffic. By default, only inbound traffic (external → internal) is uploaded. Enable this if you want visibility into outbound connections leaving your network. |
...
2. Download `CyberAlarmSecureSensor-stable.msi` from the required GitHub release.
...
If you only need to recreate the service registration without changing the token or environment, use `-Reinstall` — it reads both from the existing configuration file automatically:
...
### Optional: Verifying Image Signatures

All published images are cryptographically signed. This is an optional step that provides additional assurance that images were built by our official GitHub Actions pipeline and have not been tampered with. The installer works without this step.

You will need [Cosign](https://docs.sigstore.dev/) installed:
...
### Optional: Verifying the SBOM

The SBOM lists all components, dependencies, and versions included in the container image. Cosign is required for this step (see above).
...
CyberAlarm is developed by **Waterstons Ltd** in partnership with the **National Police Chiefs' Council (NPCC)** and **National Cyber Crime Team with City of London Police**. Technical governance and risk assessment documentation is available to authorised organisations on request.
