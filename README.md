...
> ```
> 0573a94c598cf4eeb7ae730bf46312dd66c94c2aa0783702585d80be1881e994
> ```
...
curl -fsSL https://raw.githubusercontent.com/waterstonsltd/CyberAlarmSecureSensor/refs/heads/main/installer/pca-install.sh -o pca-install.sh \
  && echo "0573a94c598cf4eeb7ae730bf46312dd66c94c2aa0783702585d80be1881e994  pca-install.sh" | sha256sum --check \
...
```
4a0015a14dcea2b05d22d53a3051c43da7f18f0e535075aeea4095cbc97b6530
```
...
Next it pulls the latest images, then verifies the signature of every image using Cosign, if installed. Verifying after the pull ensures Cosign inspects the freshly downloaded image rather than a cached version that may still carry the same tag. Any verification failure aborts the run and the service is not restarted.

The service is only restarted if at least one image digest actually changed - detected by comparing digests before and after the pull rather than relying on Docker's output format.

If the portal is unreachable the compose file and update script are both left untouched. Without a trusted hash to verify against, downloading either would offer no security guarantee. The script still proceeds to the image pull and Cosign verification steps. All activity is logged to `/var/log/cyberalarm-update.log`.
...
Compare the output against `4a0015a14dcea2b05d22d53a3051c43da7f18f0e535075aeea4095cbc97b6530` (or the current hash shown on the [CyberAlarm portal](https://member.cyberalarm.police.uk)).
