...
> ```
> 3479918326b9455869b8e37e7f5c3617b3e16bc8444f51e352f2cc97d24d3aa3
> ```
...
curl -fsSL https://raw.githubusercontent.com/waterstonsltd/CyberAlarmSecureSensor/refs/heads/main/installer/pca-install.sh -o pca-install.sh \
  && echo "3479918326b9455869b8e37e7f5c3617b3e16bc8444f51e352f2cc97d24d3aa3  pca-install.sh" | sha256sum --check \
...
```
fe22224e918dcda6773dd54862585a3003a0da87c7bff8584a44975d4d8ea9c1
```
...
echo "fe22224e918dcda6773dd54862585a3003a0da87c7bff8584a44975d4d8ea9c1  docker-compose.yaml" | sha256sum --check
...
Download the latest Windows installer (`CyberAlarmSecureSensor-win-DeploymentTool.msi`) from the [GitHub Releases](https://github.com/waterstonsltd/CyberAlarmSecureSensor/releases) page.

Run the MSI as an administrator. The installer:

1. Installs the relay as the `CyberAlarm Syslog Relay` Windows service.
2. Starts the service automatically.

Your registration token is configured via the PowerShell install script provided in the CyberAlarm portal — run it once after installation to write the token and restart the service.

The `CyberAlarmSecureSensor-win-Setup.exe`, `.nupkg`, `RELEASES`, and JSON files on the release are update assets used by Velopack. For a new Windows service installation, use the MSI deployment tool.
...
| **Installer integrity** | SHA256 hashes for the Linux installer and Docker Compose file are published and embedded at release time by the CI pipeline |
| **Windows installer provenance** | The Windows setup executable is signed with `cosign sign-blob` and published alongside its Sigstore bundle and SHA256 checksum on each release |
...
Each Windows installer release is published together with a Sigstore bundle file and SHA256 checksum file.

```bash
cosign verify-blob \
  --bundle CyberAlarmSecureSensor-win-Setup.exe.bundle \
  --certificate-identity-regexp="https://github.com/waterstonsltd/CyberAlarmSecureSensor/.*" \
  --certificate-oidc-issuer=https://token.actions.githubusercontent.com \
  CyberAlarmSecureSensor-win-Setup.exe
```
