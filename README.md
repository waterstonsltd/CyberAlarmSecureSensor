...
> ```
> 91c7a5eb723f282fd72c7b3d4257266b05f0be45d6d27df62df30b575211977c
> ```
...
curl -fsSL https://raw.githubusercontent.com/waterstonsltd/CyberAlarmSecureSensor/refs/heads/main/installer/pca-install.sh -o pca-install.sh \
  && echo "91c7a5eb723f282fd72c7b3d4257266b05f0be45d6d27df62df30b575211977c  pca-install.sh" | sha256sum --check \
...
> ```
> 13508fd93dd2e8c1faa2a3198767bfe7d515eb80085764a1779dfc6edc4e4936
> ```
> Cross-reference this against the hash shown on the [CyberAlarm portal](https://member.cyberalarm.police.uk) before running.
...
if ((Get-FileHash .\Install-CyberAlarmSecureSensor.ps1 -Algorithm SHA256).Hash.ToLowerInvariant() -ne "13508fd93dd2e8c1faa2a3198767bfe7d515eb80085764a1779dfc6edc4e4936") {
...
This removes both the installed sensor application and the Deployment Tool MSI wrapper. Uninstalling only the `CyberAlarm Secure Sensor Deployment Tool` entry from Apps & Features may leave the sensor application installed.

The PowerShell installer will:

1. Download the latest `CyberAlarmSecureSensor-win-DeploymentTool.msi` from the [GitHub Releases](https://github.com/waterstonsltd/CyberAlarmSecureSensor/releases) page.
...
You can also inspect the publisher signature manually after download:

```powershell
$sig = Get-AuthenticodeSignature .\CyberAlarmSecureSensor-win-DeploymentTool.msi
$sig.Status
$sig.SignerCertificate.Subject
$sig.TimeStamperCertificate.Subject
```

A valid installer should show `Status` = `Valid` and a signer subject containing `Waterstons`.
