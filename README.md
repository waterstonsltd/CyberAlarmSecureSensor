...
> ```
> d663a5e2f9f8c04b614e528d5b9b515f155385c9be46b3fa3db18aa69a0a654a
> ```
...
curl -fsSL https://raw.githubusercontent.com/waterstonsltd/CyberAlarmSecureSensor/refs/heads/main/installer/pca-install.sh -o pca-install.sh \
  && echo "d663a5e2f9f8c04b614e528d5b9b515f155385c9be46b3fa3db18aa69a0a654a  pca-install.sh" | sha256sum --check \
...
When you install with the `auto-update` flag, the installer downloads and installs an update script at `/opt/cyberalarm/scripts/update.sh` and registers it as a cron job that runs once per hour.
...
```
2c7b2066099da76fb2fe63f06b2b574b4de127cb37ee525ba0554a59759c8ee4
```
...
1. Downloads the latest `docker-compose.yaml` from this repository and replaces the local copy if it has changed
2. Pulls the latest `stable` image from the container registry
3. Restarts the service only if a newer image was downloaded � no unnecessary restarts

All activity is logged to `/var/log/cyberalarm-update.log`.
...
Compare the output against `2c7b2066099da76fb2fe63f06b2b574b4de127cb37ee525ba0554a59759c8ee4` (or the current hash shown on the [CyberAlarm portal](https://member.cyberalarm.police.uk)).
