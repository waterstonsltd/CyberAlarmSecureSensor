...
> **Keeping up to date:** if you are not using the auto-update flag, you are responsible for keeping the container current. Run `docker compose pull && docker compose up -d` to update.
...
docker run -d \
  ...
  ghcr.io/waterstonsltd/cyberalarm-securesensor:latest
```
...
See [LICENSE](./CyberAlarmSecureSensor?tab=License-1-ov-file) for full terms.
...
- **Security vulnerabilities:** see [SECURITY.md](./CyberAlarmSecureSensor?tab=security-ov-file)
