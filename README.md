...
> ```
> 2df960a1117cd0d55ed43ac959801a928e70bfb1f724d003bd401a38570dc3fd
> ```
...
curl -fsSL https://raw.githubusercontent.com/waterstonsltd/CyberAlarmSecureSensor/refs/heads/main/installer/pca-install.sh -o pca-install.sh \
  && echo "2df960a1117cd0d55ed43ac959801a928e70bfb1f724d003bd401a38570dc3fd  pca-install.sh" | sha256sum --check \
...
```
0d837ab892f293d09191851ceff102b2125d977c5f8c5c03c119b356158296b9
```
...
1. Fetches `hashes.json` from the CyberAlarm portal and uses `jq` to extract the authoritative SHA256 for the compose file
2. Compares the expected hash against the local compose file - if they match, no download is needed
3. If the compose file has changed, downloads and verifies it against the portal hash before replacing it
4. Verifies the signature of every image referenced in the compose file with Cosign if installed
5. Pulls the latest `stable` image and restarts the service only if a newer image was downloaded - no unnecessary restarts

If the portal is unreachable, the compose file is left untouched - without a trusted hash to verify against, downloading would offer no security guarantee. The script still proceeds to pull and verify the container image. All activity is logged to `/var/log/cyberalarm-update.log`.
...
sudo /opt/cyberalarm/scripts/update.sh \
  ...
  --compose-url https://raw.githubusercontent.com/waterstonsltd/CyberAlarmSecureSensor/refs/heads/main/installer/docker-compose.yaml
```
...
Compare the output against `0d837ab892f293d09191851ceff102b2125d977c5f8c5c03c119b356158296b9` (or the current hash shown on the [CyberAlarm portal](https://member.cyberalarm.police.uk)).
