...
> ```
> 5386c1c924641a7807a8d4f42a32a91531875491903d7edfbbd5704580dc219f
> ```
...
curl -fsSL https://raw.githubusercontent.com/waterstonsltd/CyberAlarmSecureSensor/refs/heads/main/installer/pca-install.sh -o pca-install.sh \
  && echo "5386c1c924641a7807a8d4f42a32a91531875491903d7edfbbd5704580dc219f  pca-install.sh" | sha256sum --check \
...
```
152b568ba399a4b754a2af1ea205f0ae64f590dddb61592a7be8ac3de143f4de
```
...
echo "152b568ba399a4b754a2af1ea205f0ae64f590dddb61592a7be8ac3de143f4de  docker-compose.yaml" | sha256sum --check
...
### Bulk file ingestion performance

The default Docker Compose configuration allocates **1 CPU** (`cpus: 1`), which is sufficient for continuous live syslog relay. When processing large historical log files through the drop folder, parsing throughput is CPU-bound and a single core limits throughput to roughly 20,000 events/second.

To process large files faster, increase both the CPU allocation and the parser worker count together — setting one without the other has no effect:
...
docker run -d \
  --cpus=4 \
  -e ParsingDegreeOfParallelism=4 \
  ...
```

`ParsingDegreeOfParallelism` defaults to `Environment.ProcessorCount`, so if you only set `cpus: 4` without the environment variable, the worker count will automatically match. The explicit variable is only needed if you want a different value than the CPU limit.
...
Install the `dotnet-counters` global tool if you haven't already:

```bash
dotnet tool install --global dotnet-counters
```

Then, while the app is running, execute from any directory:

```bash
dotnet-counters monitor --name CyberAlarm.SyslogRelay.ConsoleApp --counters CyberAlarm.SyslogRelay.Pipeline,CyberAlarm.SyslogRelay.Upload
```

### In Docker (using dotnet-monitor sidecar)

[dotnet-monitor](https://github.com/dotnet/dotnet-monitor) is a Microsoft-provided sidecar container that connects to your app via a shared diagnostics socket and exposes a Prometheus-compatible `/metrics` endpoint. No changes are required to the sensor image.
...
Metrics are then available at `http://localhost:52325/metrics` in Prometheus text format, ready to be scraped by Prometheus, Grafana Agent, or any compatible collector.

#### Forwarding to an OpenTelemetry backend

dotnet-monitor can push metrics directly to an OTLP endpoint (Grafana Cloud, Datadog, Azure Monitor, etc.) without requiring any OpenTelemetry packages in the sensor itself. Add the following environment variables to the `dotnet-monitor` service:
...
Consult the [dotnet-monitor documentation](https://github.com/dotnet/dotnet-monitor/blob/main/docs/configuration.md) for the full set of supported exporters and authentication options.
